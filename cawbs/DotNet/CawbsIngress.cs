...
public static class CawbsIngress
{
    ...
    public static Task<Result> PostEventAsync(string eventName, object payload, string? eventSource = null) =>
        Sess.PostEventAsync(eventName, payload, eventSource);

    public static Task<Result> GetEventStatusAsync(string actionId) =>
        Sess.GetEventStatusAsync(actionId);

    public static Task<Result> GetEventListAsync() => Sess.GetEventListAsync();

    public static Task<Result> SubmitFlagAsync(
        string name,
        string systemName,
        string sourceSystemName,
        string date) =>
        Sess.SubmitFlagAsync(name, systemName, sourceSystemName, date);
